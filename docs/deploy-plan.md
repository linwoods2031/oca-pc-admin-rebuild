...
6. 一般情况表当前只允许保存能通过前端 guard 确认归属的记录；恢复后端主记录是患者级，无法证明评估级归属时前端会中止保存，除非后续经发布负责人批准后单独放开。
7. 当前用药为空时前端不提交 `msList` 字段；恢复后端已确认 `msList` 存在时会替换用药，因此不支持通过空 `msList` 清空后端用药。
...
2. 编辑测试患者后列表和详情字段一致。
3. 未提交测试评估的一般情况表可保存或被前端按患者级共享数据风险拦截；不得出现跨患者误更新或误标记已提交评估。
4. 一般情况表空用药保存后，不应删除后端已有用药，除非用户明确删除并后端确认支持该语义。
5. 未提交测试量表可保存并刷新得分和结论。
6. 已提交评估的量表和一般情况表保存按钮不可用；即使打开后被其他端提交，保存前复查也必须拦截。
7. 回访开关只对 `<test-patient>` 验证，失败时刷新列表恢复后端状态。
8. 回访开关取消确认框、关闭确认框、接口失败后，列表均刷新并恢复后端状态。

出现任一非测试患者写入、已提交评估被写入、一般情况表跨评估误写、`msList` 误清空、归属字段异常，应立即回滚到只读包。
...
7. 患者归属字段、量表保存 payload、提交态字段、一般情况表和用药范围已从恢复后端和已认可小程序壳反查确认，见 `docs/recovered-contracts.md`。当前仓库中的 mock fixtures 只用于自动化契约预期，不代表真实生产验证结果。
8. 以下外部证据已由发布负责人补齐：
   - 线上运行包与本地恢复材料一致。
   - 一般情况表和用药属于患者级共享数据；发布负责人明确批准 PC 是否可在受限写入灰度之外编辑这些共享数据。
   - 只读灰度和受限写入灰度均有脱敏记录，且不包含真实账号、真实患者信息或真实 allow-list id。
9. 回滚包、回滚命令和回滚权限已演练，且发布负责人确认可在变更窗口内完成回退。
10. 操作审计、数据备份、生产账号权限和凭据轮换均已确认。
11. 生产静态目录仍不得直接覆盖，正式切换也必须使用 release/current 或等价可回滚软链策略。
