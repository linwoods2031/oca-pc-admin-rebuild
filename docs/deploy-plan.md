...
2. 编辑测试患者后列表和详情字段一致。
3. 未提交测试评估的一般情况表可保存，切换同一患者其他评估时不会误更新。
...
7. 以下接口契约已由后端和小程序负责人完成人工确认；当前仓库中的 mock fixtures 只用于自动化契约预期，不代表真实生产接口已确认。患者归属字段已从恢复后端和小程序壳反查确认，见 `docs/recovered-contracts.md`：
