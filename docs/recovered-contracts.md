...
- 业务用户最新确认口径要求尿失禁史等历史项使用“有/无”，恢复 SQL 和小程序显示均按该口径处理。
...
状态：`confirmed_by_latest_user_feedback_and_recovered_backend_artifacts`
...
- 恢复后端中医体质计分器会把体质判断和九种体质转化分写入报告结论字段。
- 业务用户最新确认口径要求“中医体质的转换分放到得分处”。
- 当前不改动历史数据结构，只在显示层把包含“转化分：”的中医体质结论拆分：转化分显示到“得分”，体质判断显示到“结论”。

PC 前端要求：

- 不重新计算中医体质分值。
- 仅对 `checkTableId=116` 或名称包含“中医体质”的报告做显示拆分。
- 历史对比中的“上次得分 / 上次结论”也使用同一拆分规则。

仍需在正式发布流程确认：

- 发布包对应的小程序、PC 静态包和后端 PDF 打印源码必须与本地恢复材料一致。
