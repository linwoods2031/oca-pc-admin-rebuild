...
function writePcFixture(root) {
  ...
  writeFile(root, 'src/utils/reportDisplay.js', '转化分： scoreText remarkText exScoreText exRemarkText');
}
...
function writeMiniMaintenanceFixture(root) {
  ...
  writeFile(root, 'db-hotfix/2026-05-20-feedback-batch.sql', 'oca_is_uracratia oca_is_bowel_problem 无 有');
  ...
  writeFile(root, 'backend-hotfix-src/com/qihao/udp/oca/web/controller/business/OutpatientCompositeController.java', 'TCM_TRANSFORMED_SCORE_MARKER reportScoreText previousScoreText reportRemarkText previousRemarkText');
}
...
function writeMiniShellFixture(root) {
  ...
  writeFile(root, 'pages/assessment/menu/index.js', '转化分： scoreText remarkText exScoreText exRemarkText');
  writeFile(root, 'pages/patient/list/index.wxml', '住院号 输入住院号搜索');
  writeFile(root, 'pages/patient/form/index.wxml', '修改患者 住院号');
  writeFile(root, 'pages/assessment/base/index.wxml', '居住楼层 电梯房 跌倒史 义齿 尿失禁史 便失禁史');
}
